...
import ingredientexclusion from "../assets/ingredientexclusion.json";

export function IngredientExclusion() {
    ...
    <Container className="container-shadow" style={{ width: "380px" }}>
      <Form>
        ...
        <h2 className="f-24 fw-bold mb-lg">
          Choose up to 3 ingredients that you would like to exclude
        </h2>
        <div>
          {ingredientexclusion.ingredientexclusion.map((option) => (
            <Container
              key={option.id}
              className={`option ${
